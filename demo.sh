...
#   EC2 machines are in the "modus-demo" group and tag. Inventory manages them by that.
#
...
docker run\
 ...
 --rm -it -v $(pwd):/ansible/playbooks ansible-playbook provision-ec2.yml
...
docker run\
 -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID\
 -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY\
 --rm -it -v $(pwd):/ansible/playbooks ansible-playbook systemd-units.yml
...
docker run\
 -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID\
 -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY\
 --rm -it -v $(pwd):/ansible/playbooks ansible-playbook terminate-ec2.yml
